...
public class FacebookConnect extends CordovaPlugin {
	...
	public PluginResult dialog(final JSONArray args, final CallbackContext callbackId) throws JSONException, FileNotFoundException, MalformedURLException, IOException {
		...
		Runnable runnable = new Runnable() {
			public void run() {
				me.getFacebook().dialog(me.cordova.getActivity().getApplicationContext(), method, options, new RegularDialogListener(me, callbackId));
			};
		};
		pluginResult.setKeepCallback(true);
		this.cordova.getActivity().runOnUiThread(runnable);
	...
	public PluginResult logout(final JSONArray args, final CallbackContext callbackId) throws JSONException, MalformedURLException, IOException {
		...
		this.getFacebook().logout(this.cordova.getActivity().getApplicationContext());
	...
	class AuthorizeDialogListener implements DialogListener {
		...
		public AuthorizeDialogListener(FacebookConnect me, final CallbackContext callbackId) {
			super();
		...
		public void onComplete(Bundle values) {
			...
			Thread thread = new Thread(new Runnable() {
				public void run() {
					...
					me.callbackId.sendPluginResult(pluginResult);
	...
	class RegularDialogListener implements DialogListener {
		...
		public RegularDialogListener(FacebookConnect me, final CallbackContext callbackId) {
			super();
		...
		public void onComplete(Bundle values) {
			...
			this.callbackId.sendPluginResult(pluginResult);
		}
